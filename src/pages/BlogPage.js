...
const BlogPage = () => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (!mainEl || loadingMore || visibleCount >= filteredPosts.length) return;
    ...
    const getPostThumbnail = (htmlContent) => {
        ...
        return img ? img.src : `https://placehold.co/600x400/e2e8f0/334155?text=Article`;
    };
            ...
            <div className="space-y-12">
                ...
                {loading ? (
                    ...
                    postsToShow.map((post, index) => (
                        <article key={post.id} className="animate-fade-in-up flex flex-col md:flex-row md:space-x-8 group" style={{ animationDelay: `${index * 0.1}s` }}>
                            <div className="md:w-1/3 mb-4 md:mb-0">
                            ...
                            <div className="md:w-2/3">
                                ...
                                <Link to={`/blog/${post.id}`} className="text-gray-600 dark:text-gray-400 hover:underline inline-block font-semibold">Read more...</Link>
                            </div>
                        </article>
                    ))
                ) : (
